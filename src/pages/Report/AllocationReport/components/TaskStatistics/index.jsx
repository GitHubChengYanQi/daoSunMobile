...
const TaskStatistics = ({ title }) => {
  ...
  const { loading, data, run } = useRequest(inStockOrderView, {
    onSuccess: (res) => {
      let ok = 0;
      let ing = 0;
      let revoke = 0;
      ...
      const typeTotal = type1 + type2
      setTypes([
        { number: type1, num: Math.round((type1 / typeTotal) * 100) || 0, color: '#257BDE', text: '调入' },
        { number: type2, num: 100 -(Math.round((type1 / typeTotal) * 100) || 0), color: '#FA8F2B', text: '调出' },
      ]);
    },
  });
